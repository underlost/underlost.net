...
const RecipeMeta = ({ data, settings, canonical, title, description, image, type }) => {
  ...
  description = description || data.custom_excerpt || data.meta_description || data.description || config.siteDescriptionMeta || settings.description
...
const RecipeMetaQuery = props => (
  <StaticQuery
    query={graphql`
      query GhostSettingsRecipeMeta {
        allGhostSettings {
          edges {
            node {
              ...GhostSettingsFields
    ...
    `}
    render={data => <RecipeMeta settings={data} {...props} />}
  />
)

export default RecipeMetaQuery
