...
const TwitterTimeline = ({ InitialData }: TwitterTimelineProps) => {
  ...
  const handleChangeOrdering = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    // reverse the posts array based on the selected ordering
  ...
  return (
    <>
      <div className="gh-canvas md:pb-8">
        <section className="timeline-filter mb-8 text-center">
      ...
      <VerticalTimeline>
        {posts
          .filter((post) => {
            ...
            return post.tags?.some((tag) => tag.slug === selectedTag);
          })
          .map((post, i) => {
